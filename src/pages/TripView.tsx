...
import { Loader2, Sparkles, Plus, PlaneTakeoff, CheckCircle2, ChevronRight } from "lucide-react";
...
export default function TripView() {
  ...
  if (loading) return <div className="flex h-[70vh] items-center justify-center"><Loader2 className="w-12 h-12 animate-spin text-primary opacity-50" /></div>;
      ...
      <div className="glass-card p-10 md:p-14 rounded-[3rem] shadow-xl relative overflow-hidden flex flex-col md:flex-row md:items-end justify-between gap-8">
        ...
        <div className="relative z-10 w-full">
           ...
           <div className="flex flex-wrap gap-3 mt-8">
             <span className="px-4 py-2 bg-primary/10 text-primary border border-primary/20 rounded-xl text-sm font-extrabold">{trip.temperature}</span>
             {trip.activities.map(act => (
               <span key={act} className="px-4 py-2 bg-secondary text-secondary-foreground rounded-xl text-sm font-extrabold shadow-sm">{act}</span>
             ))}
           </div>
        ...
        <div className="relative z-10 shrink-0 self-start md:self-end">
          <Button onClick={handleSuggest} disabled={aiLoading} size="lg" className="rounded-full shadow-xl shadow-fuchsia-500/20 bg-fuchsia-500 hover:bg-fuchsia-600 text-white font-extrabold h-14 px-8 transform transition-transform hover:scale-105 group">
            {aiLoading ? <Loader2 className="w-5 h-5 mr-2 animate-spin"/> : <Sparkles className="w-5 h-5 mr-3 group-hover:animate-pulse" />}
            {aiLoading ? "Analyzing Config..." : "Ask AI to Check"}
          </Button>
        </div>
      </div>
      ...
      {suggestions.length > 0 && (
        <div className="glass-card bg-fuchsia-500/5 border-fuchsia-500/20 p-8 rounded-[2.5rem] shadow-lg shadow-fuchsia-500/5 animate-in slide-in-from-top-4 duration-500 flex flex-col gap-6">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-2xl font-extrabold text-foreground">AI Packing Suggestions</h2>
          </div>
          <div className="flex flex-wrap gap-3">
            {suggestions.map(s => (
              <Button key={s} onClick={() => addSuggestionToTrip(s)} variant="outline" className="rounded-full bg-background border-fuchsia-500/30 hover:bg-fuchsia-500 hover:text-white hover:border-fuchsia-500 transition-all font-bold h-10 px-5 shadow-sm group">
      ...
      <div className="glass-card p-8 md:p-10 rounded-[3rem] shadow-xl">
        ...
        {tripItems.length === 0 ? (
           <div className="py-20 text-center flex flex-col items-center">
             <div className="h-20 w-20 bg-secondary rounded-full flex items-center justify-center mb-6"><Luggage className="w-10 h-10 text-muted-foreground opacity-50"/></div>
             <p className="text-muted-foreground text-lg font-bold">Your bag is completely empty.</p>
             <p className="text-muted-foreground/60 font-medium">Use the "Ask AI" button above to dynamically build a list!</p>
           </div>
        ) : (
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             {tripItems.map((ti, i) => {
               ...
               return (
                 <div 
                   key={ti.id} 
                   className={`group flex items-center gap-4 p-5 rounded-3xl transition-all duration-300 cursor-pointer border-2
                    ${ti.is_packed 
